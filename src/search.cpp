...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        if (limits.use_time_management() && !threads.stop && !mainThread->stopOnPonderhit)
        {
            ...
            int fallingEval = (1067 + 223 * (mainThread->bestPreviousAverageScore - bestValue)
                               + 97 * (mainThread->iterValue[iterIdx] - bestValue));
            fallingEval     = std::clamp(fallingEval, 5800, 16670);


            // If the bestMove is stable over several iterations, reduce time accordingly
            timeReduction              = lastBestMoveDepth + 8 < completedDepth ? 3.24 : 2.23;
            double reduction           = (1.48 + mainThread->previousTimeReduction) / timeReduction;
            double bestMoveInstability = 1 + 1.88 * totBestMoveChanges / threads.size();
            double recapture           = limits.capSq == rootMoves[0].pv[0].to_sq() ? 0.955 : 1.005;

            double totalTime = mainThread->tm.optimum() * (fallingEval / 10000.00) * reduction
    ...
    mainThread->previousTimeReduction = timeReduction / 2.17;
